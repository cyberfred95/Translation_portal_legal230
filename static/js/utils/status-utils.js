...
(function (global) {
  var STATUS_DEFS = [
    ...
    { raw: 'Sent to post-editing, not accepted yet', category: 'attention', label: function(lang){ return lang === 'en' ? 'Request for quote sent' : 'Demande de devis envoyée'; } },
    ...
    { raw: 'Post-edited file uploaded', category: 'completed', label: function(lang){ return lang === 'en' ? 'Proofread document uploaded' : 'Document relu importé'; } },
  ...
  function applyStatusMapping(root) {
    ...
    nodes.forEach(function (el) {
      ...
      if (badge) {
        badge.classList.remove('status-completed','status-error','status-progress','status-attention','status-default','status-post-editing');
