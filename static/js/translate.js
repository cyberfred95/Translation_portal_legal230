...
$(document).ready(function () {
                ...
                if (allTranslated && !isPostEditing) {
                    ...
                    response.forEach((project, index) => {
                        ...
                        <div class="button-set">
                            <button type="button" class="button button--secondary button--transparent button--ico download-result download-file-button" data-file="${project.translated_file}">
    ...
    <span class="button-text">Download ${project.source_file_name}</span>
</button>
    ...
    function formFileHandler(e) {
        ...
        fetch(url, {
            ...
            .then(obj => {
                if (obj.status === 200) {
                    console.log('obj.body', obj.body.project_ids)
                    let intervalId = setInterval(() => {
                        checkDocument(obj.body.project_ids, intervalId);
                    }, 10000);
                    checkDocument(obj.body.project_ids, intervalId);
                } else {
                    errorHandler();
